          ...
          <v-container fluid>
            <v-row>
              <template v-for="(name, i) in o['skos:prefLabel']">
                <v-col :md="labelColMd" cols="12" class="pdlabel primary--text text-md-right" :key="'vl'+i">{{ $t('Name') }}<template v-if="showLang && name['@language']"> ({{ name['@language'] }})</template></v-col>
                <v-col :md="valueColMd" cols="12" class="valuefield" :key="'v'+i">{{ name['@value'] }}</v-col>
              </template>
            </v-row>
            <v-row>
              <template v-for="(d, i) in o['rdfs:comment']">
                <v-col :md="labelColMd" cols="12" class="pdlabel primary--text text-md-right" :key="'dl'+i">{{ $t('Event description') }} ({{ d['@language'] }})</v-col>
            ...
            <v-row>
              <template v-for="(location, j) in o['ebucore:hasEventRelatedLocation']">
                <template v-for="(place, i) in location['skos:prefLabel']">
                  <v-col :md="labelColMd" cols="12" class="pdlabel primary--text text-md-right" :key="'dl'+i+j">{{ $t('Location') }}</v-col>
            ...
            <v-row>
              <template v-for="(id, i) in o['skos:exactMatch']">
                <v-col :md="labelColMd" cols="12" class="pdlabel primary--text text-md-right" :key="'evidl'+i">{{ getLocalizedTermLabel('entityidentifiertype', id['@type']) }}</v-col>
