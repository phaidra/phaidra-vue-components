<template>
  <v-row v-if="o['skos:prefLabel']">
    <v-col class="pdlabel primary--text text-md-right" :md="labelColMd" cols="12">{{ $t(p) }}<template v-for="(l) in o['skos:prefLabel']"><template v-if="l['@language']"> ({{ l['@language'] }})</template></template></v-col>
