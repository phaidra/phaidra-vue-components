<template>
  <span>
    <v-row v-for="(l, i) in o['skos:prefLabel']" v-if="l['@language'] === displaylang" :key="'prl'+i">
      <v-col :md="labelColMd" cols="12" v-if="p==='bf:note'" class="pdlabel primary--text text-md-right" >{{ $t(o['@type']) }}<template v-if="l['@language']"> ({{ l['@language'] }})</template></v-col>
      <v-col :md="labelColMd" cols="12" v-else class="pdlabel primary--text text-md-right">{{ $t(p) }}<template v-if="l['@language']"> ({{ l['@language'] }})</template></v-col>
