        ...
        <v-card-text class="mt-4">
          ...
          <v-layout row wrap>
            <template v-for="(volume, i) in o['bibo:volume']">
              <v-flex md3 xs12 class="primary--text" :key="'vl'+i">{{ $t('Volume') }}</v-flex>
          ...
          <v-layout row wrap>
            <template v-for="(issn, i) in o['identifiers:issn']">
              <v-flex md3 xs12 class="primary--text" :key="'isl'+i">{{ $t('ISSN') }}</v-flex>
              <v-flex md9 xs12 :key="'is'+i">{{ issn }}</v-flex>
            </template>
          </v-layout>
          <v-layout row wrap>
            <template v-for="(issued, i) in o['dcterms:issued']">
              <v-flex md3 xs12 class="primary--text" :key="'idatel'+i">{{ $t('Issued') }}</v-flex>
              <v-flex md9 xs12 :key="'idate'+i">{{ issued }}</v-flex>
            </template>
          </v-layout>
          <v-layout row wrap>
            <template v-for="(id, i) in o['skos:exactMatch']">
              <v-flex md3 xs12 class="primary--text" :key="'idatel'+i">{{ $t('Identifier') }}</v-flex>
