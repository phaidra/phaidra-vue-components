          ...
          <v-container fluid>
            <v-row>
              <template v-for="(title, j) in o['dce:title']">
                <template v-for="(mt, i) in title['bf:mainTitle']">
                  <v-col md="2" cols="12" class="pdlabel primary--text text-md-right" :key="'mt'+j+i">{{ $t(title['@type']) }}<template v-if="showLang && mt['@language']"> ({{ mt['@language'] }})</template></v-col>
            ...
            <v-row>
              <template v-for="(id, i) in o['skos:exactMatch']">
                <v-col :md="labelColMd" cols="12" class="pdlabel primary--text text-md-right" :key="'evidl'+i">{{ getLocalizedTermLabel('objectidentifiertype', id['@type']) }}</v-col>
