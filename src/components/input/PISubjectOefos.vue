      ...
      >
        <template slot="item" slot-scope="{ item }">
          <v-list-item-content two-line>
            <v-list-item-title  v-html="`${item['skos:notation'][0] + '. ' + getLocalizedTermLabel('oefos', item['@id'])}`"></v-list-item-title>
            <v-list-item-subtitle v-if="showIds" v-html="`${item['@id']}`"></v-list-item-subtitle>
          </v-list-item-content>
        </template>
        <template slot="selection" slot-scope="{ item }">
          <v-list-item-content>
            <v-list-item-title v-html="`${item['skos:notation'] + '. ' + getLocalizedTermLabel('oefos', item['@id'])}`"></v-list-item-title>
          </v-list-item-content>
        </template>
  ...
  mounted: function () {
    this.$nextTick(function () {
      if (!this.vocabularies['oefos'].loaded) {
        this.$store.dispatch('loadOefos', this.$i18n.locale)
      }
